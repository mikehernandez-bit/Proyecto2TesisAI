...
class FormatService:
    ...
    async def check_version(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"GicaTesis unavailable for version check: {e}")
    ...
    async def sync_catalog_if_needed(self, force: bool = False) -> None:
        ...
        # Attempt sync
        try:
            status, formats, etag = await self.client.list_formats(
                etag=self.cache.catalog_etag
            )
    ...
    async def list_formats(
        ...
        if university:
            formats = [
                f for f in formats 
                if f.get("university", "").lower() == university.lower()
            ]
        if category:
            formats = [
                f for f in formats 
        ...
        if document_type:
            formats = [
                f for f in formats 
                if (f.get("documentType") or "").lower() == document_type.lower()
            ]
